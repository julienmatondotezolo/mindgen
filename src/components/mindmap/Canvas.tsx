...
import type { Side, XYWH } from "@/_types/canvas";
import { Camera, CanvasMode, CanvasState, Color, LayerType, Point } from "@/_types/canvas";
import { useDisableScrollBounce, useHistory } from "@/hooks";
import {
  colorToCss,
  connectionIdToColor,
...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const layerIdsToColorSelection = useMemo(() => {
    ...
    for (const user of selections) {
      ...
      const layerId = user.selection.layerId;
  ...
  //       if (canvasState.mode !== CanvasMode.Pencil || event.buttons !== 1 || !pencilDraft) return;
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    setCamera((camera: { x: number; y: number }) => ({
      x: camera.x - e.deltaX,
      y: camera.y - e.deltaY,
    }));
  }, []);
      ...
      >
        ...
        >
          ...
          {pencilDraft && pencilDraft.length > 0 && (
            <Path fill={colorToCss(lastUsedColor)} points={pencilDraft} x={0} y={0} />
          )}
        </g>
      </svg>
    </main>
