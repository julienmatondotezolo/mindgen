...
function Account() {
  ...
  const fetchPasswordChange = useMutation(changePassword, {
    ...
    onSuccess: async () => {
      ...
      showMessage("success", "SUCCESSFUL_PASSWORD_CHANGE");
      // Reset password fields while keeping other values
      reset({
        newPassword: "",
      });
    },
    onError: (error: FetchError) => {
      ...
      if (message?.errors?.length > 0) {
        showMessage("error", message.errors[0]);
      } else {
        showMessage("error", "ERROR_PASSWORD_CHANGE");
      }
    },
  });
            ...
            <motion.section variants={itemVariants} whileHover={{ scale: 1.01 }} className="group">
              ...
              <div className="flex flex-col md:flex-row gap-6 w-full">
                <section className="flex-1">
                  <p className="text-grey dark:text-grey-blue text-sm mb-2">{profileText("currentPassword")}</p>
                  <Input
                    id="password"
                    type="password"
                    {...register("currentPassword", { required: true })}
                    placeholder={profileText("currentPassword")}
                    disabled={fetchPasswordChange.isLoading}
                    className={`transition-all duration-200 focus:ring-2 focus:ring-primary-color/50 ${
                      errors.currentPassword ? "border-red-500" : "hover:border-primary-color"
                    }`}
                  />
                  {errors.currentPassword && <span className="text-red-500 text-sm mt-1">{text("requiredInput")}</span>}
                </section>
                <section className="flex-1">
                  ...
                  <Input
                    ...
                    disabled={fetchPasswordChange.isLoading}
                    className={`transition-all duration-200 focus:ring-2 focus:ring-primary-color/50 ${
                      errors.newPassword ? "border-red-500" : "hover:border-primary-color"
                    }`}
                  />
                  {errors.newPassword && (
